...
async def configure_event_handlers(client, user_id):
    ...
    @client.on(events.NewMessage(pattern=r'^erv hastle (.+) (\d+[smhd])$'))
    ...
    @client.on(events.NewMessage(pattern=r'^erv stop$'))
    ...
    @client.on(events.NewMessage(pattern=r'^erv ping$'))
    async def ping_handler(event):
        await event.reply("🏓 Pong! Bot aktif.")

    @client.on(events.NewMessage(pattern=r'^erv bcstar (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^erv bcstargr(\d+) (\d+[smhd]) (.+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^erv stopbcstargr(\d+)$'))
    ...
    @client.on(events.NewMessage(pattern=r'^erv bl$'))
    ...
    @client.on(events.NewMessage(pattern=r'^erv unbl$'))
    ...
    @client.on(events.NewMessage(pattern=r'^erv help$'))
    async def help_handler(event):
        help_text = (
            "📋 **Daftar Perintah yang Tersedia:**\n\n"
            "1. erv hastle [pesan] [waktu][s/m/h/d]\n"
            ...
            "5. erv bcstargr [waktu][s/m/h/d] [pesan]\n"
            "   Broadcast hanya ke grup dengan interval tertentu.\n"
            "6. erv stopbcstargr[1-10]\n"
    ...
    @client.on(events.NewMessage(pattern=r'^erv setreply'))
